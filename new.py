...
def main():
    ...
    torch.manual_seed(0)
    config_path = r'Python Scripts/BYOL/config/config_linear.yaml'
    cfg = load_yaml(config_path,config_type='object') 
    
    config = yaml.load(open("Python Scripts/BYOL/config/config.yaml", "r"), Loader=yaml.FullLoader)
    ...
    #root="Python Scripts/BYOL/data", split="train+unlabeled", download=True, transform=transform)

    #train_loader = DataLoader(TRAIN_UNLABELED_DATASET,batch_size=cfg.batch_size,\
                            #drop_last=True,num_workers=0)
    
    resnet = get_model(cfg)
    ####
    #pretrained_path =  os.path.join('Python Scripts/BYOL/experiments/self-supervised/byol_unlabeled_400.pth')
        #pretrained_path =  os.path.join('Python Scripts/BYOL/experiments/test_aug/byol_small_800.pth')
    ...
    #optimizer = LARS(model.parameters(), lr=0.0003, momentum=0.99, weight_decay=1.0e-6)
...
# solver
    ...
    #write metrics to TensorBoard
        ...
        if epoch % 100 == 0:
            ...
            torch.save(resnet.state_dict(), f"Python Scripts/BYOL/experiments/self-supervised/res50_cct{epoch}.pth")


# save your improved network
    torch.save(resnet.state_dict(), "Python Scripts/BYOL/experiments/self-supervised/res50_cct.pth")
