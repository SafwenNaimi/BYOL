# -*- coding: utf-8 -*-
"""
Created on Sat May  8 10:55:12 2021
...
# you can access a dictionary using dot (.name)
class dotdict(dict):
...
# Plot images and the corresponding labels

def visualize(input_arr,labels):
    
    fig = plt.figure(figsize=(12,12))
    if len(input_arr.shape)==4: #if there is a batch size (12,3,128,128)
        num_imgs=input_arr.shape[0]
    else: ##if there is NO batch size (3,128,128)
        num_imgs=1
...
#      img = np.rollaxis(img,0,3)
      img = input_arr[i]
      img = np.rollaxis(img,0,3) #form (3,128,128) to (128,128,3) ---> (D,H,W) to (H,W,D)
